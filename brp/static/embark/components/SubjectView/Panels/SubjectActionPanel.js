...
class SubjectActionPanel extends React.Component {
    ...
    render(){
        ...
        if (record) {
            var recordUrl = '/dataentry/protocoldatasource/'+ record.pds + '/subject/' + subject.id + '/record/' + record.id + '/start/'
        }

        //https://brp.research.chop.edu/portal/dataentry/protocoldatasource/2/subject/167/record/333/start/
        return (
            <div style={divStyle} className="subject-action-panel">
                <i className="ti-close subject-action-close" onClick={this.hideActionPanel.bind(this)}></i>
                <div className="subject-action-panel-card">
                    ...
                    <button onClick={this.setEditLabelMode.bind(this)} className="btn btn-primary btn-sm record-action-btn">Edit Label</button>
                    <a href={recordUrl}><button className="btn btn-primary btn-sm record-action-btn">View Record</button></a>
                    <button onClick={this.setLinkMode.bind(this)} className="btn btn-primary btn-sm record-action-btn">Link Record</button>
                </div>
            </div>
        )
    }
